# ESP-IDF framework with its toolchains, Python environment and `idf.py`.
#
# This is a rework of mirrexagon/nixpkgs-esp-dev (CC0-1.0),
# pkgs/esp-idf/default.nix @ 5287d6e1ca9e15ebd5113c41b9590c468e1e001b:
#
# - ESP-IDF v6.0.2 (the overlay's default was v5.5.2).
# - esptool 5.x no longer depends on ecdsa, so the insecure-package
#   exemption that the overlay required is gone.
# - esp-idf-size and tree-sitter(-c) now exist in nixpkgs at versions
#   that satisfy ESP-IDF 6.0's constraints, so they are no longer pinned
#   here; python-packages.nix only pins what nixpkgs does not provide.
# - python-packages.nix is generated by update-python-packages.py from
#   the ESP-IDF checkout's own requirements + constraints files, instead
#   of being a hand-snapshotted list.
#
# Bumping ESP-IDF: set rev/sha256 below (sha256 via `nix-prefetch-url
# --type sha256` is wrong here — the fetch includes git submodules; use a
# placeholder and read the `got:` from the error), then regenerate
# python-packages.nix.
{
  owner ? "espressif",
  repo ? "esp-idf",
...
let
  ...
  allTools = callPackage ./tools.nix {
  ...
  customPython = (
    python3.withPackages (
      pythonPackages:
      let
        customPythonPackages = callPackage ./python-packages.nix { inherit pythonPackages; };
      in
      with pythonPackages;
      with customPythonPackages;
      [
        # This list is from `tools/requirements/requirements.core.txt` in the
        # ESP-IDF checkout. Names in pythonPackages come from nixpkgs; names
        # from customPythonPackages are the pins generated by
        # update-python-packages.py.
        setuptools
        packaging
        click
        pyserial
