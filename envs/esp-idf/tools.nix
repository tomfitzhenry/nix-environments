# Toolchain derivations for ESP-IDF, driven by the `tools` entry of
# `tools/tools.json` in the ESP-IDF checkout: each tool's per-platform
# tarball URL and sha256 come from the manifest Espressif publishes with
# the release, so bumping the ESP-IDF version updates the toolchains
# automatically.
#
# Adapted from mirrexagon/nixpkgs-esp-dev (CC0-1.0),
# pkgs/esp-idf/tools.nix @ 5287d6e1ca9e15ebd5113c41b9590c468e1e001b.
...
let
  ...
  mkToolDerivation =
    ...
    in stdenv.mkDerivation (finalAttrs: {
      ...
      # (the Espressif toolchains link against a Python that Nix does not provide).
